...
async function getProducts(req, res, next) {
  try {
    const result = await pool.query(`
      SELECT 
        p.id, p.name, p.category, p.unit, p.min_threshold, p.created_at,
...
async function addProduct(req, res, next) {
  try {
    const { name, category, unit, min_threshold } = req.body;
    ...
    const result = await pool.query(`
      ...
      VALUES ($1, $2, $3, $4)
      RETURNING id, name, category, unit, min_threshold, created_at
    `, [name.trim(), category.trim(), unit || 'unité', min_threshold || 0]);
...
async function updateProduct(req, res, next) {
  try {
    ...
    const result = await pool.query(`
      ...
      WHERE id = $5
      RETURNING id, name, category, unit, min_threshold, created_at
    `, [name, category, unit, min_threshold, id]);
